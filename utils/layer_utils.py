...
class AffineLayer:
    ...
    def feedforward(self, X):
        ...
        # NOTE: you MUST make sure the name here matches the one in backward(); refering to forward_out?
        self.cache['X'] = X
...
class DenseLayer(AffineLayer):
    ...
    def __init__(self, input_dim, output_dim=100, weight_scale=1e-2, activation='relu'):
        ...
        Inputs:
        - input_dim: (int) the number of input neurons, 
            like D or D1xD2x...xDn.; but not the number of samples, not N but D -Lun
    ...
    def backward(self, dout):
        ...
        ############################################################################
        dA = self.activation_backward(dout, A) # this A is right, the input of relu layer
        ...
        # from .layer_funcs import affine_backward
        # W, b = self.params['W'], self.params['b']
        # dX = affine_backward(dout * dA, X, W, b)
